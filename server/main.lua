...
local function logTransaction(identifier, description, accountName, amount, isIncome)
    ...
    if not numAmount then
        print("^1[ps-banking] Error: Invalid amount format^7")
        return false
    end

    -- Get absolute value and check range
    local absAmount = math.abs(numAmount)
    if absAmount > 999999999999999999.99 then
        print(string.format("^1[ps-banking] Error: Amount %s is too large for database (max: 999,999,999,999,999,999.99)^7", tostring(amount)))
        return false
    end

    -- Format amount to avoid scientific notation and ensure 2 decimal places
    local formattedAmount = string.format("%.2f", absAmount)
    ...
    if not success then
        print(string.format("^1[ps-banking] Error: Failed to log transaction for amount %s^7", tostring(amount)))
        return false
    end
...
RegisterNetEvent("ps-banking:server:logClient", function(account, moneyData)
    ...
    if moneyData then
        for _, data in ipairs(moneyData) do
            if data.name == "bank" then
                previousBankBalance = tonumber(data.amount) or 0
                break
            end
        end
    end

    local currentBankBalance = tonumber(getPlayerAccounts(xPlayer)) or 0
    ...
    if amountChange ~= 0 then
        ...
        -- Attempt to log the transaction
        local success = logTransaction(identifier, description, account.name, amountChange, isIncome)
        
        if not success then
            print(string.format("^1[ps-banking] Failed to log transaction for player %s (amount: %s)^7", 
                GetPlayerName(src), tostring(amountChange)))
        end
    end
end)
...
lib.callback.register("ps-banking:server:addUserToAccount", function(source, accountId, userId)
    ...
    if #accounts > 0 then
        ...
        for _, user in ipairs(users) do
            if user.identifier == targetIdentifier then
                return {
                    success = false,
